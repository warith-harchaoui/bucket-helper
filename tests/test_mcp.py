"""
Smoke test for the MCP surface (``bucket_helper.mcp``).
...
def test_mcp_module_imports_and_exposes_app():
    ...
    from bucket_helper import mcp as mcp_module

    assert hasattr(mcp_module, "app"), "bucket_helper.mcp must re-expose `app`."
    assert hasattr(mcp_module, "mcp"), "bucket_helper.mcp must expose the `mcp` handler."


def test_main_entrypoint_is_callable():
    """The ``bucket-helper-mcp`` console entry point should be a callable."""
    from bucket_helper.mcp import main
