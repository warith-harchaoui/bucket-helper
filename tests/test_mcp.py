...
Gated on the ``mcp`` extra (FastAPI + fastapi-mcp). Importing `bucket_helper.mcp`
...
from bucket_helper import mcp as mcp_module  # noqa: E402  (import mounts MCP on the app)


def test_mcp_endpoint_is_mounted() -> None:
    """Importing the module publishes an `/mcp` endpoint named 'bucket-helper'."""
    paths = {r.path for r in mcp_module.app.routes}
    assert any("/mcp" in p for p in paths), paths
    assert mcp_module.mcp.name == "bucket-helper"
